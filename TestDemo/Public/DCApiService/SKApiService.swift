...
class SKApiService: NSObject {
    ...
    typealias FailureCallBack = ((SKServiceError) -> Void)
   ...
   static func sendRequest(apiProtocol: SKApiProtocol, successCallBack: @escaping SuccessCallBack, failureCallBack: @escaping FailureCallBack) {
        ...
        provider.request(m_target, callbackQueue: apiProtocol.callBackQueue) { result in
            ...
            case .success(let response):
                ...
                if response.statusCode != DCHttpCode.successCode {
                    let error = SKServiceError(code: "\(response.statusCode)", reason: jsonStr, api: target.path)
                ...
                if code == "\(DCHttpCode.successCode)" {
                    ...
                    let service_error = SKServiceError(code: "\(code)", reason: message ?? "", api: target.path)
            ...
            case .failure(let error):
                ...
                let service_error = SKServiceError(code: "\(error.errorCode)", reason: error.errorDescription ?? "", api: target.path)
                failureCallBack(service_error)
            }
            ///判断是否需要log信息
    ...
    static func getProviderByTarget(target: TargetType) -> MoyaProvider<MultiTarget> {
